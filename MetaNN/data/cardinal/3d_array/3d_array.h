...
public:
    ...
    void SetValue(size_t p_pageId, size_t p_rowId, size_t p_colId, ElementType val)
    {
        ...
        assert((p_pageId < m_shape.PageNum()) && (p_rowId < m_shape.RowNum()) && (p_colId < m_shape.ColNum()));
        (m_mem.RawMemory())[(p_pageId * m_shape.RowNum() + p_rowId) * m_shape.ColNum() + p_colId] = val;
    }
    ...
    const auto operator () (size_t p_pageId, size_t p_rowId, size_t p_colId) const
    {
        ...
        assert((p_pageId < m_shape.PageNum()) && (p_rowId < m_shape.RowNum()) && (p_colId < m_shape.ColNum()));
        return (m_mem.RawMemory())[(p_pageId * m_shape.RowNum() + p_rowId) * m_shape.ColNum() + p_colId];
    }
