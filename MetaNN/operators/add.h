...
{
    using type = OperSeqContainer<OperAdd::NSCaseGen::Calculator>;
};

template <typename TP1, typename TP2,
          typename = std::enable_if_t<OperAdd::valid<TP1, TP2>>>
auto operator+ (TP1&& p_m1, TP2&& p_m2)
{
    return OperAdd::CreateOpTemplate(std::forward<TP1>(p_m1), std::forward<TP2>(p_m2));
}
}