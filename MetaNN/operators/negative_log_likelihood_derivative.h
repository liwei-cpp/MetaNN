...
struct OperNegativeLogLikelihoodDerivative
{
    template <typename TGrad, typename TP1, typename TP2>
    static constexpr bool valid = (IsScalar<TGrad> && IsMatrix<TP1> && IsMatrix<TP2>) ||
                                  (IsBatchScalar<TGrad> && IsBatchMatrix<TP1> && IsBatchMatrix<TP2>);
...
template <typename TGrad, typename TP1, typename TP2,
          std::enable_if_t<OperNegativeLogLikelihoodDerivative::valid<TGrad, TP1, TP2>>* = nullptr>
auto NegativeLogLikelihoodDerivative(TGrad&& p_grad, TP1&& p_tar, TP2&& p_pre)
{
    return OperNegativeLogLikelihoodDerivative
